...
public class DRFATestCase extends TestCase {
	...
	assertEquals(drfa.computeCheckPeriod(), DailyRollingFileAppender.TOP_OF_DAY);

	drfa.setDatePattern("yyyy-MM-dd mm.'log'");
	assertEquals(drfa.computeCheckPeriod(), DailyRollingFileAppender.TOP_OF_MINUTE);
	...
	assertEquals(drfa.computeCheckPeriod(), DailyRollingFileAppender.TOP_OF_HOUR);

	drfa.setDatePattern("yyyy-MM.'log'");
	assertEquals(drfa.computeCheckPeriod(), DailyRollingFileAppender.TOP_OF_MONTH);
     ...
     */
    public void test3Param(final String datePattern, final String filename) throws IOException {
	...
	try {
	    test3Param("'.'yyyy", TARGET_OUTPUT_PREFIX + "drfa_topOfYear.log");
	...
	test3Param("'.'yyyy-MM", TARGET_OUTPUT_PREFIX + "drfa_topOfMonth.log");
    }
	...
	test3Param("'.'yyyy-w", TARGET_OUTPUT_PREFIX + "drfa_topOfWeek.log");
    }
	...
	test3Param("'.'yyyy-MM-dd", TARGET_OUTPUT_PREFIX + "drfa_topOfDay.log");
    }
	...
	test3Param("'.'yyyy-MM-dd-a", TARGET_OUTPUT_PREFIX + "drfa_halfDay.log");
    }
	...
	test3Param("'.'yyyy-MM-dd-HH", TARGET_OUTPUT_PREFIX + "drfa_topOfHour.log");
    }
	...
	test3Param("'.'yyyy-MM-dd-HH-mm", TARGET_OUTPUT_PREFIX + "drfa_topOfMinute.log");
    }
	...
	assertEquals("Missing DatePattern option in rollOver().", errorHandler.getMessage(0));
    }
     ...
     */
    public void disabled_testMinuteRollover() throws IOException, InterruptedException {
	Layout layout = new SimpleLayout();
	String filename = TARGET_OUTPUT_PREFIX + "drfa_minuteRollover.log";
     ...
     */
    private static void append(final FileOutputStream combined, final FileInputStream source, final byte[] buf)
	    throws IOException {
    ...
    /**
     * Tests rollOver when log file cannot be renamed. See bug 43374.
     *
     * @throws IOException          if io error.
     * @throws InterruptedException if test interrupted while waiting for the start
     *                              of the next minute.
     */
    void DISabled_testBlockedRollover() throws IOException, InterruptedException {
	Layout layout = new SimpleLayout();
	String filename = TARGET_OUTPUT_PREFIX + "drfa_blockedRollover.log";
	String pattern = "'.'yyyy-MM-dd-HH-mm";
	...
	//
	String combinedFilename = TARGET_OUTPUT_PREFIX + "drfa_blockedRollover.combined";
    ...
    /**
     * Check that the computed rollover period for a pattern containing a week as
     * the finest unit is set to be a week. Due to a locale mismatch this was
