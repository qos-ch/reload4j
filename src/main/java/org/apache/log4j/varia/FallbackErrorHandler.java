...
/**
 *
 * The <code>FallbackErrorHandler</code> implements the ErrorHandler interface
 * such that a secondary appender may be specified. This secondary appender
 * takes over if the primary appender fails for whatever reason.
 *
 * <p>
 * The error message is printed on <code>System.err</code>, and logged in the
...
public class FallbackErrorHandler implements ErrorHandler {
    ...
    /**
     * <em>Adds</em> the logger passed as parameter to the list of loggers that we
     * need to search for in case of appender failure.
     */
