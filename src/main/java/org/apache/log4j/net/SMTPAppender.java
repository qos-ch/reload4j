 ...
 *
 * <p>
 * Since 1.2.18.3 {@link checkServerIdentity} option will check that the certificate presented by the server matches the
...
public class SMTPAppender extends AppenderSkeleton implements UnrecognizedElementHandler {
    ...
    /**
     * The <b>BufferSize</b> option takes a positive integer representing the maximum number of logging events to
     * collect in a cyclic buffer. When the
     * <code>BufferSize</code> is reached, oldest events are deleted as new events
     * are added to the buffer. By default the size of the cyclic buffer is 512 events.
     */
    ...
    /**
     * Setting the checkServerIdentity option to false will disable server identity check. By default this option is
     * enabled. However, it is only active if the SMTPProtocol is "smtps."
     *
    ...
    /**
     * This option enables server identity check and if false will disable server identity check. By default this option
     * is enabled. However, it is only active if the SMTPProtocol is "smtps."
     *
     * @param checkServerIdentity
     * @since 1.2.18.3
     */
