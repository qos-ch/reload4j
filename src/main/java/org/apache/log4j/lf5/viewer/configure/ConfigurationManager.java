...
/**
 * <p>
 * ConfigurationManager handles the storage and retrival of the state of the
 * CategoryExplorer
 *
...
public class ConfigurationManager extends Object {
	...
	if (file.exists()) {
		...
		System.err.println(
			"Unable process configuration file at " + getFilename() + ". Error Message=" + e.getMessage());
	    }
	}
	...
	// iterate through the list of log levels being used (log4j, jdk1.4, custom
	// levels)
	Iterator it = logLevelMenuItems.keySet().iterator();
	while (it.hasNext()) {
	    LogLevel level = (LogLevel) it.next();
