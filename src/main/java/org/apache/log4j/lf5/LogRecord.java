...
public abstract class LogRecord implements java.io.Serializable {
    ...
    /**
     * Abstract method. Must be overridden to indicate what log level to show in
     * red.
     */
    ...
    /**
     * Set the category associated with this LogRecord. A category represents a
     * hierarchical dot (".") separated namespace for messages. The definition of a
     * category is application specific, but a common convention is as follows:
     *
     * <p>
     * When logging messages for a particluar class you can use its class name:
     * com.thoughtworks.framework.servlet.ServletServiceBroker.<br>
     * <br>
     * Futhermore, to log a message for a particular method in a class add the
     * method name: com.thoughtworks.framework.servlet.ServletServiceBroker.init().
     * </p>
     *
    ...
    /**
     * Get the sequence number associated with this LogRecord. Sequence numbers are
     * generally assigned when a LogRecord is constructed. Sequence numbers start at
     * 0 and increase with each newly constructed LogRocord.
     *
    ...
    /**
     * Get the event time of this record in milliseconds from 1970. When a LogRecord
     * is constructed the event time is set but may be overridden by calling
     * setMillis();
     *
    ...
    /**
     * Set the event time of this record. When a LogRecord is constructed the event
     * time is set but may be overridden by calling this method.
     *
    ...
    /**
     * Get the thread description asscociated with this LogRecord. When a LogRecord
     * is constructed, the thread description is set by calling:
     * Thread.currentThread().toString(). You may supply a thread description of
     * your own by calling the setThreadDescription(String) method.
     *
    ...
    /**
     * Set the thread description associated with this LogRecord. When a LogRecord
     * is constructed, the thread description is set by calling:
     * Thread.currentThread().toString(). You may supply a thread description of
     * your own by calling this method.
     *
    ...
    /**
     * Get the stack trace in a String-based format for the associated Throwable of
     * this LogRecord. The stack trace in a String-based format is set when the
     * setThrown(Throwable) method is called.
     *
     * <p>
     * Why do we need this method considering that we have the getThrown() and
     * setThrown() methods? A Throwable object may not be serializable, however, a
     * String representation of it is. Users of LogRecords should generally call
     * this method over getThrown() for the reasons of serialization.
     * </p>
     *
     ...
     * the stack trace in a String-based format is made available via the
     * getThrownStackTrace() method.
     *
